...
const usuarioDB = JSON.parse(FS.readFileSync('db.json', 'UTF-8'));
...
isAuthenticated = ({ correo, contrasena }) => usuarioDB.users.findIndex(user => user.correo === correo && user.contrasena === contrasena) !== - 1;

// Creamos un ENDPOINT para comprobar si el usuario existe y poder crear y enviar un TOKEN
server.post('/auth/login', (req, res) => {
    const { correo, contrasena } = req.body;
    if (isAuthenticated({ correo, contrasena }) === false) {
        const status = 401;
        const message = 'Contraseña y/o contrasena incorrectos';
        res.status(status).json({ status, message })
        console.log(message);
        return;
    }
    const access_token = createToken({ correo, contrasena });
